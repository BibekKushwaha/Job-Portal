...
const JobPage = () => {
  ...
  const [statusValues, setStatusValues] = useState<Record<number, string>>({});
   ...
   const updateApplicationHandler = async (applicationId: number) => {
    const status = statusValues[applicationId];
    ...
    try {
      ...
      // Clear the status value for this specific application
      setStatusValues(prev => {
        const newValues = { ...prev };
        delete newValues[applicationId];
        return newValues;
      });
    } catch (error: any) {
                  ...
                  <div className="grid md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="flex items-center gap-3 p-4 rounded-lg border bg-background">
                      ...
                      <div>
                        ...
                        <p className="font-semibold">{Number(job.openings) % 1 === 0 
                            ? Math.floor(Number(job.openings)) 
                            : Number(job.openings)
                          } positions</p>
                      </div>
                    </div>
                ...
                {filteredApplications.map((e) => (
                  ...
                  >
                    ...
                    <div className="flex gap-2 pt-3 border-t">
                      <select
                        value={statusValues[e.application_id] || e.status}
                        onChange={(ev) => setStatusValues(prev => ({
                          ...prev,
                          [e.application_id]: ev.target.value
                        }))}
